...
// Creating variables for the water
let dist, water_geo, ground;

function init(){
    // Creating the water
    createWater(max);

    // Initializing random seed constants
    initializeConstants();
...
function divide(size, isDiamond){
...
function updateVertices(){
    ...
    for(let y = 0; y < length; ++y){
        for(let x = 0; x < length; ++x){
            if(vertices[y][x] > 0 || show_negatives){
                plane_geo.vertices[y * length + x].z = vertices[y][x];
            }
            else{
                plane_geo.vertices[y * length + x].z = 0;
            }
        }
    }
...
function createPlane(max){
    // Creating the plane geometry
    plane_geo = new THREE.PlaneGeometry(max, max, max, max);
...
function createWater(max){
    // Creating water at y = 0
    water_geo = new THREE.PlaneGeometry(max, max, max, max);
    water_geo.translate(0, 0, 0);
    water_geo.rotateX(-Math.PI / 2);
    ground = new THREE.Mesh(water_geo, water_mat);
    scene.add(ground);
}
